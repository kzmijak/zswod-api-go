...
func (aq *AttachmentQuery) QueryBlob() *BlobQuery {
	query := &BlobQuery{config: aq.config}
...
func (aq *AttachmentQuery) WithBlob(opts ...func(*BlobQuery)) *AttachmentQuery {
	query := &BlobQuery{config: aq.config}
...
//		GroupBy(attachment.FieldIconId).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (aq *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := aq.withBlob; query != nil {
		if err := aq.loadBlob(ctx, query, nodes, nil,
			func(n *Attachment, e *Blob) { n.Edges.Blob = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AttachmentQuery) loadBlob(ctx context.Context, query *BlobQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Blob)) error {
